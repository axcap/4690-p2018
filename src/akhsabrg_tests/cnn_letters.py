...
dataset_path = "res/datasets/EMNIST_LETTERS"
model_dir="res/model/cnn_letters"

embedding_dir = "res/model/embedding/"
path_for_sprites =  os.path.join(embedding_dir,'sprites.png')
path_for_metadata =  os.path.join(embedding_dir,'metadata')
...
def cnn_model_fn(features, labels, mode):
  ...
  logits = tf.layers.dense(inputs=dropout, units=27, name="output")
...
def forward(network, data):
    ...
    return class2char(predicted_class)

def class2char(class_n):
    return chr(ord('a')+class_n-1)
...
if __name__ == "__main__":
  ...
  demo = np.reshape(eval_data[-1], (1, -1))
  print("forward")
  print(forward(emnist_classifier, demo))
  imshow("Test: ", np.reshape(demo, (28,28)))
