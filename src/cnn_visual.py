...
class RNIST:
  def cnn_model_fn(self, features, labels, mode):
    ...
    logits = tf.layers.dense(inputs=conv1, units=10)
  ...
  def __init__(self, model_path="res/model/visual_demo"):
  ...
  def train(self, dataset, steps, force_retrain = False):
    old_v = tf.logging.get_verbosity()
    ...
    self.dataset_classifier.train(input_fn = train_input_fn, steps=steps)
  ...
  def main(self, unused_argv):
    ...
    dataset_classifier = tf.estimator.Estimator(
        model_fn=self.cnn_model_fn, model_dir="res/model/visual_demo")
    ...
    dataset_classifier.train(input_fn = train_input_fn, steps=10000)

    input_img_data = np.random.random((28, 28))
    print(input_img_data)

    weights = dataset_classifier.get_variable_value('conv1/kernel')
    print(weights)
    kernel = np.reshape(weights[:,:,:,0], (5,5))
    print(kernel)
    img = cv2.filter2D(input_img_data, -1, kernel)
    print(img)
    print(img.shape)
...
if __name__ == "__main__":
  ...
  nn = RNIST()
  nn.main(None)
  #nn.set_params(0.001, None, 100, 1000)
